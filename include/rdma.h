...
class RdmaResource{
    ...
    public:
        ...
        inline bool IsMaster() const {return isForMaster;}
        ...
        inline int GetCounter() {return rdma_context_counter;}
...
private:
	...
	int pending_msg = 0; //including both RDMA send and write/read that don't use the send buf
	int pending_send_msg = 0; //including only send msg
	int to_signaled_send_msg; //in order to proceed the slot_tail
	int to_signaled_w_r_msg;
...
public:
	...
	inline void* GetBase() {return (void*)vaddr;}
	...
	ssize_t Send(const void* ptr, size_t len, unsigned int id = 0, bool signaled = false);
	 ...
	 */
	ssize_t Write(raddr dest, raddr src, size_t len, unsigned int id = 0, bool signaled = false);
	ssize_t WriteWithImm(raddr dest, raddr src, size_t len, uint32_t imm, unsigned int id = 0, bool signaled = false);
	/*
	 * @dest: dest addr at local node
	 * @src: src addr at remote node
	 */
	ssize_t Read(raddr dest, raddr src, size_t len, unsigned int id = 0, bool signaled = false);

	ssize_t Cas(raddr src, uint64_t oldval, uint64_t newval, unsigned int id = 0, bool signaled = false);
...
class RdmaResourceFactory {
    ...
    public:
        inline static RdmaResource* getMasterRdmaResource(const char *devName = NULL) {
            return GetRdmaResource(true, devName);
        }
        inline static RdmaResource* getWorkerRdmaResource(const char *devName = NULL) {
